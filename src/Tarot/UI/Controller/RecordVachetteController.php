...
#[Route('/games/{id}/vachettes/new', name: 'app_game_vachette_record', methods: ['POST'])]
final class RecordVachetteController extends AbstractController
{
    ...
    public function __invoke(string $id, Request $request): Response
    {
        $user = $this->getUser();
        if (null === $user) {
            throw new \LogicException('User must be authenticated to submit a Vachette.');
        }
        ...
        if (!$form->isValid()) {
            return $this->renderVachetteForm($view, $form, null);
        }

        try {
            $this->commandBus->dispatch(new RecordVachetteCommand(
                ownerId: $user->getUserIdentifier(),
                gameId: $id,
            ...
            return $this->renderVachetteForm($view, $form, $this->errorKeyForFailedVachette($exception));
        }
     ...
     */
    private function renderVachetteForm(GameView $view, FormInterface $form, ?string $errorKey): Response
    {
