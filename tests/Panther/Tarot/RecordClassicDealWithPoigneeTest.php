...
final class RecordClassicDealWithPoigneeTest extends AbomeyPantherTestCase
{
    ...
    public function aConnectedUserAddsAPoigneeViaTheModalAndSeesItsBonusInTheCumulativeScore(): void
    {
        ...
        $crawler->filter('select[name="record_classic_deal_form[takerId]"] option[value="poig-1"]')->click();
        $crawler->filter('input[name="record_classic_deal_form[contract]"][value="garde"]')->click();
        $crawler->filter('input[name="record_classic_deal_form[bouts]"][value="1"]')->click();
